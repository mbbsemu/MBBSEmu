...
namespace MBBSEmu.HostProcess.ExportedModules {
...
#if DEBUG
      //_logger.Debug($"({Module.ModuleIdentifier}) Returning Method Offset
      //{methodPointer.Segment:X4}:{methodPointer.Offset:X4}");
#endif
    ...
    switch (ordinal) {
      ...
      default:
        throw new ArgumentOutOfRangeException(
            $"Unknown Exported Function Ordinal in PHAPI: {ordinal}");
    }
  ...
  ///
  ///     MBBSEmu exposes the entire 16-bit (4GB) address space to the module, so there is no
  ...
  private void DosAllocRealSeg() {
    ...
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Allocating {segmentSize} in Real-Mode memory at {allocatedMemorySegment}");
  ...
  ///
  ///     Allows for calling/passing information to TSR's running in real mode. This is used by some
  ///     MajorBBS modules for directly accessing the Btrieve Driver
  /// </summary>
  private void DosRealIntr() {
    ...
    var regs =
        new Regs16Struct(Module.Memory.GetArray(registerPointer, Regs16Struct.Size).ToArray());
    ...
    switch (interruptNumber) {
      ...
      case 0x7B:  // Btrieve Interrupt
      {
        ...
        switch ((EnumBtrieveOperationCodes)btvda.funcno) {
          case EnumBtrieveOperationCodes.Open: {
            ...
            var btvDataPointer =
                Module.Memory.AllocateVariable($"{fileName}-RECORD", (ushort)btvFile.RecordLength);

            var newBtvStruct =
                new BtvFileStruct { filenam = btvFileNamePointer,
                                    reclen = (ushort)btvFile.RecordLength, data = btvDataPointer };
          ...
          case EnumBtrieveOperationCodes.Stat: {
            ...
            var btvStats = new BtvstatfbStruct {
              fs = new BtvfilespecStruct() { numofr = (uint)currentBtrieveFile.GetRecordCount(),
            ...
            var definedKeys =
                currentBtrieveFile.Keys.Values.SelectMany(k => k.Segments)
                    .Select(k => new BtvkeyspecStruct() { flags = (ushort)k.Attributes,
                                                          keylen = k.Length, keypos = k.Position,
                                                          numofk = k.Number })
                    .ToList();
          ...
          default:
            throw new Exception(
                $"({Module.ModuleIdentifier}) Unknown Btrieve Operation: {(EnumBtrieveOperationCodes)btvda.funcno}");
        }
      ...
      default:
        throw new Exception(
            $"({Module.ModuleIdentifier}) Unhandled Interrupt: {interruptNumber:X2}h");
    }
