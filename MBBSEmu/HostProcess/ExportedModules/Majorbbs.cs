...
namespace MBBSEmu.HostProcess.ExportedModules {
  ...
  public class Majorbbs : ExportedModuleBase, IExportedModule {
    ...
    public enum TFStateCodes : ushort { TFSDUN = 0, TFSBGN = 1, TFSBOF = 2, TFSLIN = 3, TFSEOF = 4 }
    ...
    /// </summary>
    private readonly Dictionary<Guid, IEnumerator<string>> _activeSearches =
    ...
    public new void Dispose() { _tfsStreamReader?.Dispose();

    base.Dispose();
  }

  public Majorbbs(IClock clock, IMessageLogger logger, AppSettingsManager configuration,
                  IFileUtility fileUtility, IGlobalCache globalCache, MbbsModule module,
                  PointerDictionary<SessionBase> channelDictionary,
                  IAccountKeyRepository accountKeyRepository, IAccountRepository accountRepository,
                  ITextVariableService textVariableService)
      : base(clock, logger, configuration, fileUtility, globalCache, module, channelDictionary,
             textVariableService) {
    ...
    _int21h = new Int21h(Registers, Module.Memory, _clock, _logger, _fileFinder, null, null,
                         new TextWriterStream(Console.Error), Module.ModulePath);
    ...
    var usraccPointer = Module.Memory.AllocateVariable(
        "USRACC", (ushort)(UserAccount.Size * _numberOfChannels), true);
    ...
    Module.Memory.AllocateVariable("MODULE", (FarPtr.Size * 0xFF),
                                   true);  // Array of Module Info Pointers
    ...
    Module.Memory.SetArray("LANGUAGES",
                           new LingoStruct().Data);  // ever user will have the same lingo struct
    ...
    Module.Memory.AllocateVariable("TXTVARS", TextvarStruct.Size * MaxTextVariables,
                                   true);  // Up to 64 Text Variables per Module
    Module.Memory.SetArray(
        "TXTVARS", new TextvarStruct("SYSTEM", new FarPtr(Segment, 9000))
                       .Data);  // Set 1st var as SYSTEM variable reference with special pointer
    Module.Memory.AllocateVariable("HDLCON",
                                   FarPtr.Size);  // Handles the connection for the current User
    Module.Memory.AllocateVariable(
        "RANDSEED", sizeof(uint));  // Seed value for Borland C++ Pseudo-Random Number Generator
    Module.Memory.SetDWord("RANDSEED",
                           (uint)_random.Next(1, ushort.MaxValue));  // Seed RNG with a random value
    ...
    for (var i = 0; i < 256; i++) {
      ...
      Module.Memory.SetByte(ctypePointer.Segment, (ushort)(ctypePointer.Offset + i + 1),
                            (byte)resultByte);
    }
  }
  ...
  public void SetState(ushort channelNumber) {
    ...
    var vdaChannelPointer =
        Module.Memory.GetOrAllocateVariablePointer($"VDA-{channelNumber}", VOLATILE_DATA_SIZE);
    ...
    Module.Memory.SetWord(Module.Memory.GetVariablePointer("STATUS"),
                          (ushort)ChannelDictionary[channelNumber].GetStatus());
    ...
    Module.Memory.SetArray(currentExtUserAccPointer,
                           ChannelDictionary[channelNumber].ExtUsrAcc.Data);
    Module.Memory.SetArray(Module.Memory.GetVariablePointer("EXTPTR"),
                           currentExtUserAccPointer.Data);
    ...
    var channelFsdscb = Module.Memory.GetOrAllocateVariablePointer($"FSD-Fsdscb-{ChannelNumber}",
                                                                   FsdscbStruct.Size);

    Module.Memory.SetPointer("*FSDSCB", channelFsdscb);

    // Processing Channel Input
    if (ChannelDictionary[channelNumber].GetStatus() ==
        EnumUserStatus.CR_TERMINATED_STRING_AVAILABLE)
      ProcessChannelInput(channelNumber);
  }

  /// <summary>
  ///     Handles processing of Channel Input by setting the INPUT value of the dta in the channel
  ///     InputCommand buffer and then calling parsin()
  /// </summary>
  /// <param name="channelNumber"></param>
  ...
  public void UpdateSession(ushort channel) {
    ...
    var resultStatus =
        (EnumUserStatus)Module.Memory.GetWord(Module.Memory.GetVariablePointer("STATUS"));

    // If STATUS was changed programatically, queue it up
    if (resultStatus != ChannelDictionary[ChannelNumber].GetStatus())
      ChannelDictionary[ChannelNumber].Status.Enqueue(resultStatus);

    // Save off the USRPTR area to the Channel
    var userPointer = Module.Memory.GetVariablePointer("USER");
    ChannelDictionary[ChannelNumber].UsrPtr.Data =
        Module.Memory
            .GetArray(userPointer.Segment, (ushort)(userPointer.Offset + (User.Size * channel)),
                      User.Size)
            .ToArray();

    // We Verify it exists as Unit Tests won't call SetState() which would establish the VDA for
    // that Channel
    if (Module.Memory.TryGetVariablePointer($"VDA-{ChannelNumber}", out var vdaPointer))
      ChannelDictionary[ChannelNumber].VDA =
          Module.Memory.GetArray(vdaPointer, VOLATILE_DATA_SIZE).ToArray();

#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) {channel}->status == {ChannelDictionary[ChannelNumber].GetStatus()}");
    _logger.Debug(
        $"({Module.ModuleIdentifier}) {channel}->state == {ChannelDictionary[ChannelNumber].UsrPtr.State}");
    _logger.Debug(
        $"({Module.ModuleIdentifier}) {channel}->substt == {ChannelDictionary[ChannelNumber].UsrPtr.Substt}");
#endif
  }
  ...
  public ReadOnlySpan<byte> Invoke(ushort ordinal, bool offsetsOnly = false) {
    ...
    switch (ordinal) {
      ...
      case 339:
        hexopt();
        break;
      case 450:  // onsysn() = same functionality as onsys(), adds invisible user parameter which we
                 // ignore
      case 449:
        onsys();
        break;
      ...
      default:
        _logger.Error(
            $"Unknown Exported Function Ordinal in MAJORBBS: {ordinal}:{Ordinals.MAJORBBS[ordinal]}");
  ...
  private bool anpbtv(FarPtr recordPointer, bool caseSensitive,
                      EnumBtrieveOperationCodes operationCodes) {
    ...
    var lastKeyAsString =
        Encoding.ASCII.GetString(Module.Memory.GetString(btvStruct.key, stripNull: true));
  ...
  private void time() {
    ...
    var passedSeconds =
        (int)(_clock.Now - new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalSeconds;
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Passed seconds: {passedSeconds} (AX:{Registers.AX:X4}, DX:{Registers.DX:X4})");
#endif
  }
  ...
  private void gmdnam() {
    ...
    var variablePointer =
        Module.Memory.GetOrAllocateVariablePointer("GMDNAM", (ushort)moduleName.Length);

    // Set Memory
    Module.Memory.SetArray(variablePointer, Encoding.Default.GetBytes(moduleName));
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Retrieved Module Name \"{moduleName}\" and saved it at host memory offset {variablePointer}");
#endif
...
#if DEBUG
      //_logger.Debug($"({Module.ModuleIdentifier}) Copied {inputBuffer.Length} bytes from
      //{sourcePointer} to {destinationPointer} -> {Encoding.ASCII.GetString(inputBuffer)}");
#endif
    }
  ...
  private void stzcat() {
    ...
    if (stringLength > destinationBufferLength) {
      ...
      Module.Memory.SetByte(destinationPointer.Segment,
                            (ushort)(destinationPointer.Offset + destinationBufferLength), 0);
      Registers.SetPointer(destinationPointer);
      return;
    }
  ...
  private void register_module() {
    ...
    var moduleStruct =
        new ModuleStruct(Module.Memory.GetArray(destinationPointer, ModuleStruct.Size));

    // We create a copy in Variable Memory in case the MODULE pointer that was passed in was on the
    // stack
    var localModuleStructPointer = Module.Memory.AllocateVariable(
        $"MODULE-LOCAL-{Module.ModuleDlls[ModuleDll].StateCode}", ModuleStruct.Size);
    Module.Memory.SetArray($"MODULE-LOCAL-{Module.ModuleDlls[ModuleDll].StateCode}",
                           moduleStruct.Data);
    ...
    Module.Memory.SetPointer(Module.Memory.GetVariablePointer("MODULE") + moduleStructOffset,
                             localModuleStructPointer);

#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) MODULE pointer ({Module.Memory.GetVariablePointer("MODULE") + moduleStructOffset}) set to {localModuleStructPointer}");
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Module Description set to {Module.ModuleDescription}");
#endif
  }
  ...
  private void opnmsg() {
    ...
    if (!McvPointerDictionary.Any(x => x.Value.FileName == mcvFileName)) {
      ...
      _ = new MsgFile(
          _fileFinder, Module.ModulePath,
          mcvFileName.Replace(".mcv", string.Empty, StringComparison.InvariantCultureIgnoreCase));
    }

    var offset =
        McvPointerDictionary.Allocate(new McvFile(_fileFinder, mcvFileName, Module.ModulePath));
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Opened MCV file: {mcvFileName}, assigned to {ushort.MaxValue:X4}:{offset:X4}");
#endif
  ...
  private void numopt() {
    ...
    if (outputValue < floor || (ceiling > 0 && outputValue > ceiling))
      throw new ArgumentOutOfRangeException(
          $"{msgnum} value {outputValue} is outside specified bounds");
  ...
  ///
  ///     Functionally, as far as this helper method is concerned, there's no difference between
  ///     this method and stgopt()
  ///
  ...
  private void l2as() {
    ...
    var variablePointer = Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("L2AS"),
                                                                     0x20);  // 32 Byte Buffer
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Received value: {outputValue}, string saved to {variablePointer}");
#endif
...
#if DEBUG
      _logger.Warn(
          $"({Module.ModuleIdentifier}) Unable to cast {stringToLong} ({GetParameterPointer(0)}) to long");
#endif
    }
  }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Copied {sourceString.Length} bytes from {sourcePointer} to {destinationPointer}");
#endif
  ...
  private void uacoff() {
    ...
    var variablePointer =
        Module.Memory.GetOrAllocateVariablePointer($"USRACC-{userNumber}", UserAccount.Size);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Added {formattedMessage.Length} bytes to the buffer");
#endif
  }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Sent {outputBuffer.Length} bytes to Channel {userChannel}");
#endif
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Deducted {creditsToDeduct} from the current users account (Ignored)");
#endif
  ...
  private void prfmsg() {
    ...
    if (!McvPointerDictionary[_currentMcvFile.Offset].Messages.TryGetValue(messageNumber,
    ...
    var currentPrfPositionPointer =
        Module.Memory.GetPointer(Module.Memory.GetVariablePointer("PRFPTR"));
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Added {formattedMessage.Length} bytes to the buffer from message number {messageNumber}");
#endif
    // Update Pointer
  ...
  private ReadOnlySpan<byte> channel {
    get {
      ...
      foreach (var k in ChannelDictionary.Keys) {
        Array.Copy(BitConverter.GetBytes((ushort)k), 0, arrayOutput, k + (2 * channelsWritten++),
                   2);
      }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Added {Encoding.Default.GetString(string2)} credits to user account {Encoding.Default.GetString(string1)} (unlimited -- this function is ignored)");
#endif
  }

  /// <summary>
  ///     Converts an integer value to a null-terminated string using the specified base and stores
  ///     the result in the array given by str
  ///
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Convterted integer {integerValue} to {output} (base {baseValue}) and saved it to {string1Pointer}");
#endif
  }
  ...
  private void haskey() {
    ...
    if (_accountRepository.GetAccountByUsername(ChannelDictionary[ChannelNumber].Username) ==
        null) {
      keys = _configuration.DefaultKeys;
    } else {
      var accountKeys =
          _accountKeyRepository.GetAccountKeysByUsername(ChannelDictionary[ChannelNumber].Username);
      keys = accountKeys.Select(x => x.accountKey);
    }

    Registers.AX =
        keys.Any(k => string.Equals(accountLock, k, StringComparison.InvariantCultureIgnoreCase))
            ? (ushort)1
            : (ushort)0;
...
#if DEBUG
      _logger.Debug(
          $"User {ChannelDictionary[ChannelNumber].Username} was denied access -- {lockName} key is required");
#endif
    }
#if DEBUG
  ...
  private void hasmkey() {
    ...
    var accountLock =
        Encoding.ASCII.GetString(McvPointerDictionary[_currentMcvFile.Offset].GetString(msgnum))
            .TrimEnd('\0');
...
#define ddday(date)    ((date)    &0x001F)
    ...
    var variablePointer = Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("NCDATE"),
                                                                     (ushort)outputDate.Length);
  ...
  private void ncedat() {
    ...
    var variablePointer =
        Module.Memory.GetOrAllocateVariablePointer("NCEDAT", (ushort)outputDate.Length);
  ...
  private void setmbk() {
    ...
    if (mcvFilePointer.Segment != ushort.MaxValue &&
        !McvPointerDictionary.ContainsKey(mcvFilePointer.Offset))
...
#if DEBUG
      _logger.Debug(
          $"({Module.ModuleIdentifier}) Enqueue Previous MCV File: {McvPointerDictionary[_currentMcvFile.Offset].FileName} (Pointer: {_currentMcvFile})");
#endif
    }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Set Current MCV File: {McvPointerDictionary[_currentMcvFile.Offset].FileName} (Pointer: {mcvFilePointer})");
#endif
  }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Reset Current MCV to {McvPointerDictionary[_currentMcvFile.Offset].FileName} ({_currentMcvFile}) (Queue Depth: {_previousMcvFile.Count})");
#endif
  }
  ...
  private void opnbtv() {
    ...
    var btrieveFile = new BtrieveFileProcessor(_fileFinder, Module.ModulePath, btrievefileName,
                                               _configuration.BtrieveCacheSize);
  ...
  [VisibleForTesting]
  public FarPtr AllocateBB(BtrieveFileProcessor btrieveFile, ushort maxRecordLength,
                           string fileName) {
    // Setup Pointers
    var btvFileStructPointer =
        Module.Memory.AllocateVariable($"{fileName}-STRUCT", BtvFileStruct.Size);
    ...
    var newBtvStruct = new BtvFileStruct { filenam = btvFileNamePointer, reclen = maxRecordLength,
                                           data = btvDataPointer, key = btvKeyPointer };
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Opened file {fileName} and allocated it to {btvFileStructPointer}");
#endif
...
#if DEBUG
    var btvStruct =
        new BtvFileStruct(Module.Memory.GetArray(btrieveFilePointer, BtvFileStruct.Size));
    var btvFileName =
        Encoding.ASCII.GetString(Module.Memory.GetString(btvStruct.filenam, stripNull: true));
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Setting current Btrieve file to {btvFileName} ({btrieveFilePointer})");
#endif
  }

  private bool UpdateBB(BtrieveFileProcessor currentBtrieveFile, FarPtr destinationRecordBuffer,
                        EnumBtrieveOperationCodes operationCode,
                        short keyNumber = -1) => UpdateBB(currentBtrieveFile,
                                                          destinationRecordBuffer,
                                                          currentBtrieveFile.Position,
                                                          operationCode.AcquiresData(), keyNumber);

  private bool UpdateBB(BtrieveFileProcessor currentBtrieveFile, FarPtr destinationRecordBuffer,
                        uint position, bool acquiresData, short keyNumber = -1) {
    ...
    if (keyNumber >= 0 && currentBtrieveFile.Keys.Count > 0)
      Module.Memory.SetArray(
          btvStruct.key,
          currentBtrieveFile.Keys[(ushort)keyNumber].ExtractKeyDataFromRecord(record.Data));
  ...
  private bool updateBtv() {
    ...
    var dataToWrite = Module.Memory.GetArray(btrieveRecordPointerPointer,
                                             (ushort)currentBtrieveFile.RecordLength);
  ...
  private bool insertBtv(LogLevel logLevel) {
    ...
    var dataToWrite =
        Module.Memory.GetArray(btrieveRecordPointer, (ushort)currentBtrieveFile.RecordLength);
...
#if DEBUG
    //_logger.Debug($"({Module.ModuleIdentifier}) Added {formattedMessage.Length} bytes to the
    // buffer: {Encoding.ASCII.GetString(formattedMessage)}");
#endif
  ...
  private void scnmdf() {
    ...
    var variablePointer =
        Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("SCNMDF"), 0xFF);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Set {numberOfBytesToWrite} bytes at {destinationPointer} with {(byte)byteToWrite:X2}");
#endif
  }
  ...
  ///
  ///     Because it's a pointer, the segment only contains Int16:Int16 pointer to the actual prfbuf
  ///     segment
  ///
  ...
  private void register_textvar() {
    ...
    // Save
    Module.Memory.SetArray(Module.Memory.GetVariablePointer("TXTVARS") + newTextVarOffset,
                           newTextVar.Data);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Registered Textvar \"{name}\" to {functionPointer} ({Module.Memory.GetVariablePointer("TXTVARS") + newTextVarOffset})");
#endif
  }
  ...
  private bool obtainBtv(FarPtr recordPointer, FarPtr keyPointer, int keyNumber,
                         EnumBtrieveOperationCodes obtopt) {
    var currentBtrieveFile = BtrieveGetProcessor(Module.Memory.GetPointer("BB"));

    var keyValue = !keyPointer.IsNull()
                       ? Module.Memory.GetArray(
                             keyPointer, (ushort)currentBtrieveFile.Keys[(ushort)keyNumber].Length)
                       : null;
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Volatile Memory Size requested of {size} bytes ({VOLATILE_DATA_SIZE} bytes currently allocated per channel)");
#endif
  }
  ...
  ///
  ///     Because UserNumber and Channels in MBBSEmu are the same, we can just use the usernum AS
  ///     channel
  ///
  ...
  private void vdaoff() {
    ...
    var volatileMemoryAddress =
        Module.Memory.GetOrAllocateVariablePointer($"VDA-{channel}", VOLATILE_DATA_SIZE);
  ...
  private void sameto() {
    ...
    Registers.AX =
        (ushort)(string2Buffer.StartsWith(string1Buffer, StringComparison.CurrentCultureIgnoreCase)
                     ? 1
                     : 0);
  }

  /// <summary>
  ///     Compares ending of strings, checking if the first string ends with the second string
  ///     (ignoring case)
  ///
  ...
  private void samend() {
    ...
    Registers.AX =
        (ushort)(string1Buffer.EndsWith(string2Buffer, StringComparison.CurrentCultureIgnoreCase)
                     ? 1
                     : 0);
  }
  ...
  private void cncchr() {
    ...
    Module.Memory.SetPointer("NXTCMD",
                             new FarPtr(nxtcmdPointer.Segment, (ushort)(nxtcmdPointer.Offset)));
  }
  ...
  private void f_open() {
    ...
    fileStructPointer ??= Module.Memory.GetOrAllocateVariablePointer(
        $"FILE_{fullPath}-{FilePointerDictionary.Count}", FileStruct.Size);
    ...
    fileStream ??=
        File.Open(fullPath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) {fullPath} FILE struct written to {fileStructPointer}");
#endif
...
#if DEBUG
      _logger.Warn(
          $"({Module.ModuleIdentifier}) Called FCLOSE on null File Stream Pointer (0000:0000), usually means it tried to open a file that doesn't exist");
      Registers.AX = 0xFFFF;
      return;
#endif
    }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Closed File {filePointer} {fileStream.Name} (Stream: {fileStruct.curp})");
#endif
  ...
  /// <summary>
  ///     Looks for the specified filename (filespec) in the BBS directory, returns 1 if the file is
  ///     there
  ///
  ...
  private void fnd1st() {
    ...
    try {
      var fileEnumerator = Directory.EnumerateFileSystemEntries(
          path, search, FileUtility.CASE_INSENSITIVE_ENUMERATION_OPTIONS);
  ...
  private void fndnxt() {
    ...
    if (!_activeSearches.TryGetValue(fndblk.Guid, out var enumerator)) {
      _logger.Warn(
          $"({Module.ModuleIdentifier}) Called fndnxt but the GUID wasn't found {fndblk.Guid}");
      return;
    }
  ...
  ///
  ///     Signature: size_t fread(void* ptr, size_t elementSize, size_t numberOfElements, FILE*
  ///     stream)
  /// </summary>
  private void f_read() {
    var destinationPointer = GetParameterPointer(0);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Evaluated string length of {stringValue.Length} for string at {GetParameterPointer(0)} ({stringValue})");
#endif
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Converted {(char)character} to {(char)Registers.AX}");
#endif
  }
  ...
  private void rename() {
    ...
    try {
      File.Move(Path.Combine(Module.ModulePath, oldFilenameInputValue),
                Path.Combine(Module.ModulePath, newFilenameInputValue), true);

#if DEBUG
      _logger.Debug(
          $"({Module.ModuleIdentifier}) Renamed file {oldFilenameInputValue} to {newFilenameInputValue}");
#endif
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Filled {numberOfBytesToFill} bytes at {destinationPointer} with {(byte)valueToFill:X2}");
#endif
  }
  ...
  private void getmsg() {
    ...
    var variablePointer =
        Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("GETMSG"), 0x1000);
    var outputValue = McvPointerDictionary[_currentMcvFile.Offset].GetString(msgnum);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Retrieved option {msgnum} from {McvPointerDictionary[_currentMcvFile.Offset].FileName} (MCV Pointer: {_currentMcvFile}), saved {outputValue.Length} bytes to {variablePointer}");
#endif
  ...
  /// <summary>
  ///     Reads data from s and stores them accounting to parameter format into the locations given
  ...
  private static IEnumerator<char> FromFileStream(FileStream input) {
    int b;
    while ((b = input.ReadByte()) >= 0) yield return Convert.ToChar(b);
  }

  /// <summary>
  ///     Reads data from stream and stores them accounting to parameter format into the locations
  ...
  private enum ScanfParseState { NORMAL, PERCENT }
  ;
  ...
  private int GetNumberBaseFromCharacter(char formatChar) => formatChar switch { 'x' => 16,
                                                                                 'o' => 8,
                                                                                 _ => 10 };
    ...
    foreach (var formatChar in formatString) {
      ...
      switch (parseState.State) {
        ...
        case ScanfParseState.PERCENT when Char.IsAsciiDigit(formatChar):
          ...
          break;
        case ScanfParseState.PERCENT when formatChar == 'i' || formatChar == 'd' ||
            formatChar == 'u' || formatChar == 'x':
          var result = GetLeadingNumberFromString(input, GetNumberBaseFromCharacter(formatChar),
                                                  parseState.Length > 0 ? parseState.Length : -1);
          moreInput = result.MoreInput;
          if (parseState.IsLongInteger) {
            // low word first followed by high word
            Module.Memory.SetWord(GetParameterPointer(startingParameterOrdinal),
                                  (ushort)((uint)result.Value & 0xFFFF));
            Module.Memory.SetWord(GetParameterPointer(startingParameterOrdinal) + 2,
        ...
        case ScanfParseState.PERCENT when formatChar == 's' || formatChar == 'c':
          ...
          (var stringValue, moreInput) = ReadString(input, c => {
            return (count++ < length) ? ExportedModuleBase.CharacterAccepterResponse.ACCEPT
        ...
        case ScanfParseState.PERCENT:
          throw new ArgumentException(
              $"({Module.ModuleIdentifier}) Unsupported sscanf specifier: {formatChar}");
      }
    }
  ...
  private void prfmlt() {
    ...
    var currentPrfPositionPointer =
        Module.Memory.GetPointer(Module.Memory.GetVariablePointer("PRFPTR"));
    Module.Memory.SetArray(currentPrfPositionPointer, formattedMessage);
    currentPrfPositionPointer.Offset +=
        (ushort)(formattedMessage.Length - 1);  // dont count the null terminator

#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Added {output.Length} bytes to the buffer (Message #: {messageNumber})");
#endif
    // Update Pointer
    Module.Memory.SetPointer("PRFPTR", currentPrfPositionPointer);
  }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Registered Global Command Handler at: {globalCommandHandlerPointer}");
#endif
  }
  ...
  private void catastro() {
    ...
    _logger.Error(
        $"({Module.ModuleIdentifier}) Catastro Failure: {Encoding.ASCII.GetString(formattedMessage)}");
  ...
  private void fgets() {
    ...
    if (!FilePointerDictionary.TryGetValue(fileStruct.curp.Offset, out var fileStream))
      throw new FileNotFoundException(
          $"Unable to locate FileStream for {fileStructPointer} (Stream: {fileStruct.curp})");

    if (fileStream.Position == fileStream.Length) {
      _logger.Warn(
          $"({Module.ModuleIdentifier}) Attempting to read EOF file, returning null pointer");
      Registers.AX = 0;
      Registers.DX = 0;
      return;
    }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Wrote {formattedMessage.Length} bytes to {fileStructPointer} (Stream: {fileStruct.curp})");
#endif
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Seek to {fileStream.Position} in {fileStructPointer} (Stream: {fileStruct.curp})");
#endif
  ...
  private void nctime() {
    // From DOSFACE.H:
    // #define dttime(hour,min,sec) (((hour)<<11)+((min)<<5)+((sec)>>1))
    // var packedTime = (_clock.Now.Hour << 11) + (_clock.Now.Minute << 5) + (_clock.Now.Second >>
    // 1);
    ...
    var variablePointer = Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("NCTIME"),
                                                                     (ushort)timeString.Length);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Received value: {packedTime}, decoded string {timeString} saved to {variablePointer}");
#endif
    Registers.SetPointer(variablePointer);
  }
  ...
  private void strchr() {
    ...
    for (var i = 0; i < stringToSearch.Length; i++) {
      if (stringToSearch[i] != (byte)characterToFind)
        continue;
  ...
  private void parsin() {
    ...
    var inputComponents = Encoding.ASCII.GetString(Module.Memory.GetString("INPUT")).Split(' ');
    ...
    Module.Memory.SetPointer(margvPointer,
                             inputPointer);  // Set 1st command to start at start of input
    ...
    for (var i = 0; i < parsedInput.Length; i++) {
      ...
      while (i < parsedInput.Length && parsedInput[i] == 0) i++;
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Moved {bytesToMove} bytes {sourcePointer}->{destinationPointer}");
#endif
  }
  ...
  private void instat() {
    ...
    var userSession = ChannelDictionary.Values.FirstOrDefault(
        x => userId.SequenceEqual(StringFromArray(x.UsrAcc.userid).ToArray()));
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) User Found -- Channel {userSession.Channel}, user[] offset {userBase}");
#endif
    Registers.AX = 1;
  }
  ...
  private void samein() {
    ...
    Registers.AX =
        (ushort)(stringToSearch.Contains(stringToFind, StringComparison.InvariantCultureIgnoreCase)
                     ? 1
                     : 0);
  }

  /// <summary>
  ///     Skip past whitespace, returns pointer to the first NULL or non-whitespace character in the
  ///     string
  ///
  ...
  private void skpwht() {
    ...
    for (var i = 0; i < stringToSearch.Length; i++) {
      if (stringToSearch[i] == 0x0 || stringToSearch[i] == 0x20)
        continue;
  ...
  /// <summary>
  ///     Just like the standard fgets(), except it uses '\r' as a line terminator (a hard carriage
  ///     return on The Major BBS), and it won't have a line terminator on the last line if the file
  ///     doesn't have it.
  ///
  ...
  private void mdfgets() {
    ...
    if (!FilePointerDictionary.TryGetValue(fileStruct.curp.Offset, out var fileStream))
      throw new Exception(
          $"({Module.ModuleIdentifier}) Unable to locate FileStream for {fileStructPointer} (Stream: {fileStruct.curp})");
  ...
  private void fsdroom() {
    ...
    if (!Module.Memory.TryGetVariablePointer($"FSD-TemplateBuffer-{ChannelNumber}",
                                             out var fsdBufferPointer))
      fsdBufferPointer =
          Module.Memory.AllocateVariable($"FSD-TemplateBuffer-{ChannelNumber}", 0x2000);

    if (!Module.Memory.TryGetVariablePointer($"FSD-FieldSpec-{ChannelNumber}",
                                             out var fsdFieldSpecPointer))
      fsdFieldSpecPointer =
          Module.Memory.AllocateVariable($"FSD-FieldSpec-{ChannelNumber}", 0x2000);
    ...
    if (!Module.Memory.TryGetVariablePointer($"FSD-Fsdscb-{ChannelNumber}", out var channelFsdscb))
      channelFsdscb =
          Module.Memory.AllocateVariable($"FSD-Fsdscb-{ChannelNumber}", FsdscbStruct.Size);

    if (!Module.Memory.TryGetVariablePointer($"FSD-Fsdscb-{ChannelNumber}-newans",
                                             out var newansPointer))
    ...
    if (!Module.Memory.TryGetVariablePointer($"FSD-Fsdscb-{ChannelNumber}-flddat",
                                             out var fsdfldPointer))
      fsdfldPointer = Module.Memory.AllocateVariable($"FSD-Fsdscb-{ChannelNumber}-flddat",
                                                     FsdfldStruct.Size * 100);

    var fsdStatus = new FsdscbStruct(Module.Memory.GetArray(channelFsdscb, FsdscbStruct.Size)) {
      fldspc = fsdFieldSpecPointer, flddat = fsdfldPointer, newans = newansPointer
    };
  ...
  private void stlcpy() {
    ...
    for (var i = inputBuffer.Length; i < limit; i++) inputBuffer.WriteByte(0x0);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Assigned Polling Routine {ChannelDictionary[channelNumber].PollingRoutine} to Channel {channelNumber}");
#endif
  }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Unassigned Polling Routine on Channel {channelNumber}");
#endif
  }
  ...
  private void stpans() {
    ...
    var resultPointer = Module.Memory.GetOrAllocateVariablePointer(
        GetLocalVariableName("STPANS"), 1920);  // Max Screen Size of 80x24
  ...
  private void qrybtv() {
    ...
    var key = Module.Memory.GetArray(keyPointer, (ushort)currentBtrieveFile.Keys[keyNumber].Length);
  ...
  private void gabbtv() {
    ...
    UpdateBB(currentBtrieveFile, recordPointer, (uint)absolutePosition, acquiresData: true,
             (short)keynum);
  }
  ...
  private void strtok() {
    ...
    if (!stringToSplitPointer.Equals(FarPtr.Empty)) {
      ...
      Module.Memory.SetWord(
          lengthPointer,
          (ushort)(stringToSplitPointer.Offset +
                   Module.Memory.GetString(stringToSplitPointer, stripNull: true).Length));
    }
    ...
    var stringDelimiter =
        Encoding.ASCII.GetString(Module.Memory.GetString(stringDelimitersPointer, stripNull: true));

    // skip starting delimiters
    while (workPointer.Offset < endOffset &&
           stringDelimiter.Contains((char)Module.Memory.GetByte(workPointer))) {
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Wrote {stringToWrite.Length} bytes from {GetParameterPointer(0)}, written to {fileStructPointer} (Stream: {fileStruct.curp})");
#endif
    Registers.AX = 1;
  }
  ...
  private void rawmsg() {
    ...
    var variablePointer =
        Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("RAWMSG"), 0x1000);
    var outputValue = McvPointerDictionary[_currentMcvFile.Offset].GetString(msgnum);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Retrieved option {msgnum} from {McvPointerDictionary[_currentMcvFile.Offset].FileName} (MCV Pointer: {_currentMcvFile}): {outputValue}");
#endif
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Read 1 byte from {fileStructPointer} (Stream: {fileStruct.curp}): {characterRead:X2}");
#endif
  ...
  private void stricmp() {
    var string1 = GetParameterPointer(0) == FarPtr.Empty ? string.Empty
                                                         : GetParameterString(0, stripNull: true);
    var string2 = GetParameterPointer(2) == FarPtr.Empty ? string.Empty
                                                         : GetParameterString(2, stripNull: true);
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Allocated {size} bytes starting at {allocatedMemory}");
#endif
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Character {(char)character} written to {fileStructPointer} (Stream: {fileStruct.curp})");
#endif
    Registers.AX = 1;
  }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Created Big Memory Region that is {qty} records of {size} bytes in length at {bigRegion}");
#endif
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Retrieved Big Memory block {bigRegionPointer}, returned pointer to index {index}: {indexPointer}");
#endif
  ...
  private void extoff() {
    ...
    var variablePointer =
        Module.Memory.GetOrAllocateVariablePointer($"EXTUSR-{userNumber}", ExtUser.Size);
  ...
  ///     Borland C++ Macro
  ///
  ///     This macro with functional form fills env with information about the current state of the
  ///     calling environment in that point of code execution, so that it can be restored by a later
  ///     call to longjmp.
  ///
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Jump Set -- {jmpBuf.cs:X4}:{jmpBuf.ip:X4} AX:{Registers.AX}");
#endif
  }
  ...
  private void endcnc() {
    ...
    var remainingCharactersInCommand =
        inputLength - (nxtcmdPointer.Offset - inputPointer.Offset) + 1;
    ...
    var remainingInput =
        Module.Memory.GetArray(nxtcmdPointer, (ushort)(remainingCharactersInCommand));
  ...
  private void morcnc() {
    ...
    for (var i = 0; i < remainingCharactersInCommand; i++) {
      ...
      Module.Memory.SetPointer(
          "NXTCMD", new FarPtr(nxtcmdPointer.Segment, (ushort)(nxtcmdPointer.Offset + i)));
      return;
    }
  ...
  private void cncint(CncIntegerReturnType returnType) {
    ...
    var inputString = Encoding.ASCII.GetString(
        Module.Memory.GetArray(nxtcmdPointer, (ushort)remainingCharactersInCommand));
  ...
  private void qnpbtv() {
    ...
    var currentBtrieveFile = BtrieveGetProcessor(Module.Memory.GetPointer("BB"));
    var result = currentBtrieveFile.PerformOperation(
        currentBtrieveFile.LastUsedKey, currentBtrieveFile.LastUsedKeyData, queryOption);
    if (result)
  ...
  private void memcpy() {
    ...
    if (sourcePointer.Segment >= 0xFF00) {
      _logger.Warn(
          $"Attempting to copy from Unmapped Exported Module. Source Pointer: {sourcePointer}");
      _logger.Warn(
          "Because this action is unsupported, it might cause issues with further Module execution.");
      Registers.SetPointer(destinationPointer);
      return;
    } else if (destinationPointer.Segment >= 0xFF00) {
      _logger.Warn(
          $"Attempting to copy to Unmapped Exported Module. Destination Pointer: {sourcePointer}");
      _logger.Warn(
          "Because this action is unsupported, it might cause issues with further Module execution.");
      Registers.SetPointer(destinationPointer);
      return;
  ...
  ///     pointed by ptr2, returning zero if they all match or a value different from zero
  ...
  private void tokopt() {
    ...
    for (var i = 1; i < ushort.MaxValue; i += 2) {
      ...
      try {
        ...
        _logger.Warn(
            $"Error Reading string at {messagePointer}, terminating list at {tokenList.Count} elements");
        break;
      }
    }
  ...
  private void dostounix() {
    ...
    var specifiedDate = new DateTime(dateStruct.year, dateStruct.month, dateStruct.day,
  ...
  private void register_agent() {
    ...
    _galacticommClientServerAgent =
        new AgentStruct(Module.Memory.GetArray(agentPointer, AgentStruct.Size));
  }
  ...
  private void msgscan() {
    ...
    var msgScanResultPointer =
        Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("MSGSCAN"), 0x1000);
  ...
  private void getdtd() {
    ...
    var packedTime =
        (ushort)((fileTime.Hour << 11) + (fileTime.Minute << 5) + (fileTime.Second >> 1));
    var packedDate = (ushort)((fileTime.Month << 5) + fileTime.Day + ((fileTime.Year - 1980) << 9));

#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Returned Packed Date for {filePointer.Name} ({fileTime}) AX: {packedTime} DX: {packedDate}");
#endif
  ...
  private void cntdir() {
    ...
    var pathString =
        _fileFinder.ResolvePathWithWildcards(Module.ModulePath, GetParameterFilename(0));
  ...
  /// <summary>
  ///     Retrieves a C-string containing the value of the environment variable whose name is
  ...
  private void getenv() {
    ...
    var resultPointer =
        Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("GETENV"), 0xFF);
  ...
  private void ftgsbm() {
    ...
    var fileSpec = new FtgStruct(
        Module.Memory.GetArray(Module.Memory.GetVariablePointer("FTG"), FtgStruct.Size));

    // Call TSHBEG to get file to send
    // Decrement the initial Stack Pointer enough to ensure it wont overwrite anything in ththate
    // current stack space
    Module.Execute(
        fileSpec.tshndl, ChannelNumber, true, true,
    ...
    var fileToSend = Encoding.ASCII.GetString(
        Module.Memory.GetString(Module.Memory.GetVariablePointer("TSHMSG"), true));
    ...
    ChannelDictionary[ChannelNumber].SendToClient(
        File.ReadAllBytes(Path.Combine(Module.ModulePath, fileToSend)));

    // Call TSHFIN to get file to send
    // Decrement the initial Stack Pointer enough to ensure it wont overwrite anything in the
    // current stack space
    Module.Execute(
        fileSpec.tshndl, ChannelNumber, true, true,
  ...
  private void listing() {
    ...
    ChannelDictionary[ChannelNumber].SendToClientRaw(
        File.ReadAllBytes(Path.Combine(Module.ModulePath, fileToSend)));

    Module.Execute(finishedFunctionPointer, ChannelNumber, true, true, null,
                   (ushort)(Registers.SP - 0x800));
  }
  ...
  ///
  ///     The clock function returns the processor time elapsed since the beginning of the program
  ///     invocation.
  ///
  ...
  ///     Opens a new file for reading/writing
  ///
  ///     This method differs from f_open in that it doesn't create a FILE struct, it only returns
  ///     the handle
  ///
  ...
  private void mkdir() {
    ...
    var directoryName = _fileFinder.FindFile(
        Module.ModulePath,
        Encoding.ASCII.GetString(Module.Memory.GetString(directoryNamePointer, true)));
  ...
  private void getftime() {
    ...
    var packedTime = (ushort)((info.CreationTime.Hour << 11) + (info.CreationTime.Minute << 5) +
                              (info.CreationTime.Second >> 1));
    var packedTimeData = BitConverter.GetBytes(packedTime);

    var packedDate =
        (ushort)(((_clock.Now.Year - 1980) << 9) + (_clock.Now.Month << 5) + _clock.Now.Day);
  ...
  private void fsdapr() {
    ...
    var fsdAnswersPointer =
        Module.Memory.GetOrAllocateVariablePointer($"FSD-Answers-{ChannelNumber}", 0x800);
  ...
  private void strtol() {
    ...
    var stringContainingLongs =
        Encoding.ASCII.GetString(Module.Memory.GetString(stringPointer, stripNull: true));
    ...
    if (stringContainingLongs == "") {
      ...
      if (suffixPointer != FarPtr.Empty)
        Module.Memory.SetPointer(suffixPointer,
                                 new FarPtr(stringPointer.Segment, stringPointer.Offset));

      return;
    }

    var longToParse = stringContainingLongs.Split(' ', StringSplitOptions.RemoveEmptyEntries)[0];
    var longToParseLength =
        longToParse.Length +
        (stringContainingLongs.Length - stringContainingLongs.TrimStart(' ')
                                            .Length);  // We do this as length might change with
                                                       // logic below and add back in leading spaces
    ...
    if (suffixPointer != FarPtr.Empty)
      Module.Memory.SetPointer(suffixPointer,
                               new FarPtr(stringPointer.Segment,
                                          (ushort)(stringPointer.Offset + longToParseLength + 1)));
  }
  ...
  private void fsdbkg() {
    ...
    ChannelDictionary[ChannelNumber].SendToClient(
        FormatNewLineCarriageReturn(Module.Memory.GetString(templatePointer)));
  }
  ...
  private void fsdrhd() {
    ...
    var ripHeaderPointer =
        Module.Memory.GetOrAllocateVariablePointer($"FSD-RIPHeader-{ChannelNumber}", 0xFF);
  ...
  private void fsdego() {
    ...
    var fsdFieldVerificationRoutinePointer = Module.Memory.GetOrAllocateVariablePointer(
        $"FSD-FieldVerificationRoutine-{ChannelNumber}", FarPtr.Size);
    var fsdWhenDoneRoutinePointer = Module.Memory.GetOrAllocateVariablePointer(
        $"FSD-WhenDoneRoutine-{ChannelNumber}", FarPtr.Size);
    ...
    var fsdscbStruct =
        new FsdscbStruct(Module.Memory.GetArray(fsdscbStructPointer, FsdscbStruct.Size));
    fsdscbStruct.fldvfy = fieldVerificationPointer;
    Module.Memory.SetArray(fsdscbStructPointer, fsdscbStruct.Data);

#if DEBUG
    _logger.Debug(
        $"Channel {ChannelNumber} entering Full Screen Display (v:{fieldVerificationPointer}, d:{whenDoneRoutinePointer}");
#endif
  }
  ...
  private void vfyadn() {
    ...
    if (fieldNo == fsdscbStruct.numtpl - 1) {
      ...
      switch (fsdscbStruct.xitkey) {
        ...
        {
          Module.Memory.SetArray("FSDEMG",
                                 Encoding.ASCII.GetBytes("Are you sure? Enter QUIT to quit now\0"));
          Registers.AX = (ushort)EnumFsdStateCodes.VFYCHK;
          break;
        }
        default:
  ...
  private void fsdnan() {
    ...
    Module.Memory.SetArray(fsdnanPointer,
                           Encoding.ASCII.GetBytes(fsdStatus.Fields[fieldNo].Value + '\0'));

#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Retrieved Field {fieldNo}: {fsdStatus.Fields[fieldNo].Value} ({fsdnanPointer})");
#endif
  ...
  private void sortstgs() {
    ...
    for (var i = 0; i < numberOfStrings; i++) {
      ...
      listOfStrings.Add(new Tuple<FarPtr, string>(
          destinationPointer,
          Encoding.ASCII.GetString(Module.Memory.GetString(destinationPointer, true))));
    }
  ...
  private void lastwd() {
    ...
    var stringToParse =
        Encoding.ASCII.GetString(Module.Memory.GetString(stringPointerBase, stripNull: true));
  ...
  private void findtvar() {
    ...
    for (ushort i = 0; i < MaxTextVariables; i++) {
      var currentTextVar = new TextvarStruct(
          Module.Memory.GetArray(txtvarMemoryBase + (i * TextvarStruct.Size), TextvarStruct.Size));
  ...
  ///     quick interval. This might have been added to handle sub-second routines not running on
  ///     DOS, and not having the DOS Interrupt ability that RTIHDLR used.
  ///
  ...
  private void aabbtv() {
    ...
    Registers.AX = UpdateBB(currentBtrieveFile, recordPointer, (uint)absolutePosition,
                            acquiresData: true, (short)keynum)
                       ? (ushort)1
                       : (ushort)0;
  }
  ...
  private void uidkey() {
    ...
    Registers.AX =
        (ushort)(keys.Any(k => string.Equals(accountLock, k,
    ...
    var lockName = Encoding.ASCII.GetString(Module.Memory.GetString(GetParameterPointer(0), true));
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Returning {Registers.AX} for uidkey({userName}, {lockName})");
#endif
  }
  ...
  private void othkey() {
    ...
    if (_accountRepository.GetAccountByUsername(ChannelDictionary[userChannel].Username) == null) {
      ...
      var accountKeys =
          _accountKeyRepository.GetAccountKeysByUsername(ChannelDictionary[userChannel].Username);
      keys = accountKeys.Select(x => x.accountKey);
    }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Returning {Registers.AX} for othkey({accountLock})");
#endif
  }
  ...
  private void ul2as() {
    ...
    var resultPointer = Module.Memory.GetOrAllocateVariablePointer(GetLocalVariableName("UL2AS"),
                                                                   0x20);  // 32 Byte Buffer
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Received value: {outputValue}, string saved to {resultPointer}");
#endif
  ...
  private void cncyesno() {
    ...
    switch (inputStringComponents[0]) {
      ...
      case "N":
        ...
        break;
      default:
        Registers.AX = string.IsNullOrEmpty(inputStringComponents[0]) ? (ushort)0
                                                                      : inputStringComponents[0][0];
        return;
    }

    Module.Memory.SetPointer(
        "NXTCMD", new FarPtr(inputPointer.Segment,
                             (ushort)(inputPointer.Offset + inputStringComponents[0].Length + 1)));
  }
  ...
  private void printf() {
    ...
    _logger.Info(
        Encoding.ASCII.GetString(FormatPrintf(Module.Memory.GetString(formatStringPointer), 2)));
  }
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Returning {Registers.AX} for gen_haskey({accountLock})");
#endif
  }
  ...
  private void cncwrd() {
    ...
    var returnPointer =
        Module.Memory.GetOrAllocateVariablePointer("CNCWRD", 0x1E);  // max length is 30 characters
  ...
  private void pltile() {
    ...
    for (var i = 0; i < numberOfSegments; i++) Module.Memory.AllocateRealModeSegment(tsize);

    _logger.Debug(
        $"({Module.ModuleIdentifier}) Allocated base {realSegmentBase} for {size} bytes ({numberOfSegments} segments)");
  ...
  private void onsys() {
    ...
    Registers.AX =
        ChannelDictionary.Any(x => string.Equals(x.Value.Username, username,
  ...
  private void dcdate() {
    ...
    Registers.AX = (ushort)((inputDateTime.Month << 5) + inputDateTime.Day +
                            ((inputDateTime.Year - 1980) << 9));
  }
  ...
  ///
  ///     Because MBBSEmu doesn't put restrictions on User-ID Characters, we'll verify it's a
  ///     standard, printable ASCII value (between 32 & 126)
  ///
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Setting Echo Security ON for {ChannelDictionary[ChannelNumber].ExtUsrAcc.wid} characters with the character {(char)ChannelDictionary[ChannelNumber].ExtUsrAcc.ech}");
#endif
  }
  ...
  /// </summary>
  private void condex() {
    if (!ChannelDictionary[ChannelNumber].UsrPtr.Flags.IsFlagSet((ushort)EnumRuntimeFlags.Concex))
      return;
    Registers.Halt = true;
  ...
  private void hdluid() {
    ...
    // Look up user ID
    var userAccount = _accountRepository.GetAccounts().ToList().FirstOrDefault(
        item => item.userName.Contains(searchUserName, StringComparison.CurrentCultureIgnoreCase));
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Allocated {inputBuffer.Length} bytes starting at {destinationAllocatedPointer}");
#endif
  ...
  private void cncsig() {
    ...
    var returnPointer =
        Module.Memory.GetOrAllocateVariablePointer("CNCSIG", 0xA);  // max length is 10 characters
    Registers.SetPointer(returnPointer);
  ...
  /// <summary>
  ///     Technically TXTVARS is a struct, which holds the system text variables and the offset of
  ///     the *(varrour)() to be executed.
  ///
  ///     We set he pointer of the delegate to this routine, which will return the value of the
  ///     variable looked up by a proceeding FINDTVAR() call
  /// </summary>
  ...
  private void setmode() {
    ...
    var fileStructPointer = Module.Memory.GetOrAllocateVariablePointer(
        $"FILE_{fileStreamPointer.Name}-{FilePointerDictionary.Count}", FileStruct.Size);
    var fileStruct = new FileStruct(Module.Memory.GetArray(fileStructPointer, FileStruct.Size));
...
#if DEBUG
    _logger.Debug(
        $"({Module.ModuleIdentifier}) Setting mode {fileMode} for {fileStreamPointer.Name}");
#endif
  }
  ...
  ///     Full Screen Editor
  ///
  ///     We re-use the FSD, using a pre-defined custom template, field spec, and formats (in Assets
  ///     Folder)
  /// </summary>
  private void bgnedt() {
    ...
    var answersString = new StringBuilder(
        $"TOPIC={Encoding.ASCII.GetString(Module.Memory.GetString(topicPointer))}",
        topicLength + textLength);

    // Take the input text and split it out on new lines so we can hydrate the individual lines on
    // the template
    ...
    var fsdBufferPointer =
        Module.Memory.GetOrAllocateVariablePointer($"FSD-TemplateBuffer-{ChannelNumber}", 0x2000);
    var fsdFieldSpecPointer =
        Module.Memory.GetOrAllocateVariablePointer($"FSD-FieldSpec-{ChannelNumber}", 0x2000);
    ...
    var channelFsdscb = Module.Memory.GetOrAllocateVariablePointer($"FSD-Fsdscb-{ChannelNumber}",
                                                                   FsdscbStruct.Size);
    var newansPointer =
        Module.Memory.GetOrAllocateVariablePointer($"FSD-Fsdscb-{ChannelNumber}-newans", 0x400);

    // Declare flddat -- allocating enough room for up to 100 fields
    var fsdfldPointer = Module.Memory.GetOrAllocateVariablePointer(
        $"FSD-Fsdscb-{ChannelNumber}-flddat", FsdfldStruct.Size * 100);
    ...
    var fsdAnswersPointer =
        Module.Memory.GetOrAllocateVariablePointer($"FSD-Answers-{ChannelNumber}", 0x800);
    Module.Memory.SetArray($"FSD-Answers-{ChannelNumber}",
                           Encoding.ASCII.GetBytes(answersString.ToString()));
    ...
    // Set Exit Routine
    var fsdWhenDoneRoutine = Module.Memory.GetOrAllocateVariablePointer(
        $"FSD-WhenDoneRoutine-{ChannelNumber}", FarPtr.Size);
    ...
    // Because control has been handed over to the FSD, we need to stop execution of the module
    // routine
    Registers.Halt = true;
  }

  /// <summary>
  ///     Expect a Variable Length User-ID (only valid User-ID Characters up to a valid User-ID
  ///     Length)
  ///
  ...
  private void cncuid() {
    ...
    // Iterate through nxtcmd until we hit an invalid character or reach the limit of the uid number
    // of characters
