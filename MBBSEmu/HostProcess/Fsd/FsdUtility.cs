...
namespace MBBSEmu.HostProcess.Fsd {
  ...
  public class FsdUtility : IFsdUtility {
    /// <summary>
    ///     Parses a Field Spec String for FSD and returns a strongly typed list of the Field
    ///     Specifications
    /// </summary>
    /// <param name="fieldSpec"></param>
    /// <returns></returns>
    public List<FsdFieldSpec> ParseFieldSpecs(ReadOnlySpan<byte> fieldSpec) {
      if (fieldSpec.IsEmpty || fieldSpec.Length == 0)
      ...
      for (var i = 0; i < fieldSpec.Length; i++) {
        ...
        if (fieldSpec[i] == '(') {
          ...
          for (var o = i; o < fieldSpec.Length; o++) {
            ...
            var fieldOption = Encoding.ASCII.GetString(fieldSpec.Slice(i, o - i))
                                  .ToUpper()
    ...
    /// <summary>
    ///     Determines the X,Y coordinates of specified template fields by stripping ANSI from the
    ///     field template and parsing each character, each new line increments Y and sets X back
    ///     to 1.
    ///
      ...
      for (var i = 0; i < templateString.Length; i++) {
        ...
        switch (c) {
          case '!':  // Error Message Field
            ...
            status.ErrorField = new FsdFieldSpec { FsdFieldType = EnumFsdFieldType.Error,
    ...
    /// <summary>
    ///     Takes a memory block of null terminated strings in a double null terminated list and
    ...
    public List<string> ParseAnswers(int answerCount, ReadOnlySpan<byte> answerList) {
      ...
      throw new Exception(
          "Unable to parse FSD Answers. Found fewer Answers than the number of Answers Specified");
    }
    ...
    public ReadOnlySpan<byte> StripAnsi(ReadOnlySpan<byte> inputBuffer) {
      ...
      foreach (var c in inputBuffer) msResult.WriteByte(c < 127 ? c : (byte)0x20);

      var templateWithoutAnsi =
          new Regex(@"\x1b\[[0-9;]*m")
              .Replace(Encoding.ASCII.GetString(msResult.ToArray()), string.Empty);
    ...
    public ReadOnlySpan<byte> BuildAnswerString(FsdStatus fsdStatus) {
      using var result = new MemoryStream(
          fsdStatus.Fields.Sum(field => field.Name.Length + field.Value.Length + 4));
